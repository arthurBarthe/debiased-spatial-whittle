...
from debiased_spatial_whittle.likelihood import DebiasedWhittle, MultivariateDebiasedWhittle, Estimator
...
HypothesisTestResult = namedtuple('HypothesisTestResult',
                                  (
                                      'lkh_ratio',
                                      'max_eig',
                                      'p_value',
...
class FixedParametersHT:
    ...
    """
    def __init__(self, full_model: CovarianceModel, null_parameters: dict,
    ...
    def __call__(self, z: np.array, level=0.05):
        ...
        w_matrix = z.shape[0] * z.shape[1] / 2 * np.matmul(j_full, - h_null_inv + h_full_inv)
        ...
        print(test)
        return HypothesisTestResult(lkh_ratio, np.max(np.abs(eigenvalues)), p_value, test)