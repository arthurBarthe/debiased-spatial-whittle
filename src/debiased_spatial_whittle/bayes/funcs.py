...
def compute_gradient(
    func: Callable, x: ndarray, approx_grad: bool = False, **func_kwargs
) -> ndarray:
    """
...
def fit(
    ...
    **opt_kwargs,
    ...
    if (
        not is_func and not func.transform_flag and bounds is None
    ):  # assumes func has .transform_lag property
        # print('sdfsdfa')
        bounds = func.model.param_bounds[
            : len(x0)
        ]  # assumes func has .model.param_bounds method/property
    else:
        bounds = None
    ...
    def obj(x):
        return -func(x, **loglik_kwargs)  # minimize negative
    ...
    if basin_hopping:  # for global optimization
        ...
        res = minimize(
            x0=x0, fun=obj, jac=gradient, method="L-BFGS-B", bounds=bounds, **opt_kwargs
        )
        success = res["success"]
...
def RW_MH(niter, x0, log_posterior, propcov, acceptance_lag=1000, **logpost_kwargs):
    ...
    props = h * np.random.multivariate_normal(
        np.zeros(len(propcov)), propcov, size=niter
    )
...
def compute_propcov_original(
    logpost: Callable, x: ndarray, approx_grad: bool = False
) -> ndarray:
