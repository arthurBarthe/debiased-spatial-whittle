...
class Likelihood(ABC):
    def __init__(
        ...
        nugget: Optional[float] = 0.1,
        use_taper: Union[None, ndarray] = None,
        ...
        if self.model.name == "TMultivariateModel":  # TODO: better way to do this?
            ...
            self.model.name == "SquaredSamplerOnRectangularGrid"
        ):  # TODO: this is not name of model
    ...
    def cov_func(
        self, params: ndarray, lags: Union[None, List[ndarray]] = None, **cov_args
    ) -> List[ndarray]:
    ...
    def fit(
        ...
        **opt_kwargs,
        ...
        def obj(x):
            return (
                -1 / (self.n_points * self.constant) * self(x, **loglik_kwargs)
            )  # minimize rescaled negative loglik
        ...
        if basin_hopping:  # for global optimization
            ...
            res = basinhopping(
                obj, x0, minimizer_kwargs=minimizer_kwargs, **opt_kwargs
            )  # niter!!
    ...
    def sim_J_matrix(
        ...
        **opt_kwargs,
        ...
        while niter > i:
            ...
            # TODO: func kwargs?
            grad_at_params = compute_gradient(
                obj, params, approx_grad=approx_grad, **loglik_kwargs
            )
        ...
        self.Jhat = np.cov(
            self.grad_at_params.T
        )  # multiply by (2/N) for original dewhittle constants
        return self.grad_at_params

    def sim_H_and_J_matrices(
        self,
        params: ndarray,
        niter: int = 5000,
        print_res: bool = True,
        ...
        **opt_kwargs,
        ...
        i = 0
        self.grad_at_params = np.zeros((niter, self.n_params), dtype=np.float64)
        self.hess_at_params = np.zeros(
            (niter, self.n_params, self.n_params), dtype=np.float64
        )
        while niter > i:
            z = self.sim_z(params)
            loglik_kwargs = {"z": z}
            ...
            hess_at_params = compute_hessian(
                obj, params, approx_grad=approx_grad, **loglik_kwargs
            )

            if not np.all(np.isfinite(grad_at_params)) or np.all(
                np.isfinite(hess_at_params)
            ):
                continue

            if print_res:
                print(
                    f"\riteration {i+1})", end=""
                )  # grad: {grad_at_params.round(3)}')
            self.grad_at_params[i] = grad_at_params
            self.hess_at_params[i] = hess_at_params
            i += 1

        self.Hhat = -np.mean(
            self.hess_at_params, axis=0
        )  # multiply by (2/N) for original dewhittle constants
    ...
    def compute_C2(self):
        ...
        M = cholesky(H_)
        self.C2 = (
            inv(M) @ M_A / np.sqrt(self.n_points / 2)
        )  # re-adjusting to 1/2 scale!
        return
    ...
    def compute_C5(self, mle: ndarray):
        ...
        propcov = compute_hessian(
            self, mle, inv=True
        )  # this is inv(H), inv of observed fisher
