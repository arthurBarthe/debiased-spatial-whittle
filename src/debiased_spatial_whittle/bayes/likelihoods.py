...
class DeWhittle(Likelihood):
    ...
    def __call__(
        self, params: ndarray, z: Optional[ndarray] = None, **cov_args
    ) -> float:
    ...
    def d_expected_periodogram(self, params: ndarray) -> ndarray:
        ...
        for i in range(len(params)):
            ...
            d_ep.append(
                compute_ep(aux, self.grid.spatial_kernel(), self.grid.mask, fold=True)
            )  # TODO: ask about spatial_kernel and grid.mask
    ...
    def fisher(self, params: ndarray) -> ndarray:
        ...
        self.H = (
            1 / self.n_points * H
        )  # not using self.constant. Always rescaling to make consistent with Jhat
        return self.H
...
class Whittle(Likelihood):
    def __init__(
        ...
        model: CovarianceModel,
        nugget: float,
        use_taper: Optional[ndarray] = None,
    ...
    def freq_grid(self):
        ...
        return np.meshgrid(
            *(2 * np.pi * fftfreq(n) for n in self.n), indexing="ij"
        )  # / (delta*n)?
    ...
    def aliased_grid(self):
        ...
        return np.meshgrid(
            *(
                2 * np.pi * np.arange(-(n // 2), n // 2 + 1) / delta[i]
                for i, n in enumerate(shape)
            ),
            indexing="ij",
        )  # np.arange(0,1) for non-aliased version
    ...
    def freq_grid_for_fft(self):
        ...
        return np.stack(
            np.meshgrid(*(np.arange(-n // 2, n // 2) for n in self.n), indexing="ij")
        )
...
class Gaussian(Likelihood):
    def __init__(
        ...
        super().__init__(
            z, grid, model, nugget=nugget, use_taper=None, transform_func=transform_func
        )
