...
class Optimizer:
    def __init__(
        self, likelihood: Likelihood, transform_func: Optional[Callable] = None
    ):
...
class MCMC:
    def __init__(self, likelihood: Likelihood, prior: Prior):
        assert (
            likelihood.n_params == prior.n_params
        ), "dimensions of likelihood and prior not equal"
    ...
    def RW_MH(
        ...
        **logpost_kwargs,
        ...
        if adjusted:
            ...
            def posterior(x):
                return self.adj_logpost(x, **logpost_kwargs)
            ...
            def posterior(x):
                return self.logpost(x, **logpost_kwargs)

            label = self.likelihood.label

        # TODO: use optimizer propcov!!!
        propcov = compute_hessian(
            posterior, self.likelihood.res.x, approx_grad=approx_grad, inv=True
        )
        ...
        props = h * np.random.multivariate_normal(
            np.zeros(self.n_params), propcov, size=niter
        )
