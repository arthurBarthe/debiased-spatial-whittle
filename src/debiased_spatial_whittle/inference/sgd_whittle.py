...
Note: This implementation does NOT use FFTs. Instead, it computes the periodogram and
Expected periodogram directly for specific frequency indices, making it suitable for
...
class StochasticDebiasedWhittle:
    ...
    This computes the likelihood for a batch of frequency indices without using FFT,
    making it suitable for stochastic gradient descent optimization.

    The key difference from the standard Debiased Whittle Likelihood is that we compute
    the periodogram and expected periodogram directly for specific frequency indices,
    rather than computing them for all frequencies using FFT.
    ...
    def compute_periodogram_at_indices(
        self, sample: SampleOnRectangularGrid, indices: List[tuple[int, ...]]
    ) -> torch.Tensor:
        """
        Compute the periodogram at specific frequency indices without using FFT.

        The periodogram at frequency k is given by:
        I(k) = |sum_x z(x) * exp(-i * k^T * x)|^2 / n

        where z(x) are the data values, k is the frequency index, x are the spatial locations,
        ...
        """
        # Get sample values and ensure they are torch tensors
        values = sample.values
        if not isinstance(values, torch.Tensor):
            values = torch.tensor(values, dtype=torch.float64)

        # Get grid information
        grid_shape = sample.grid.n
        ndim = len(grid_shape)
        n_points = torch.prod(torch.tensor(grid_shape, dtype=torch.float64))

        # Get Fourier frequencies for the grid
        fourier_freqs = sample.grid.fourier_frequencies
        ...
        # For each spatial dimension, create coordinates
        coords = []
        for dim in range(ndim):
            coord = torch.arange(grid_shape[dim], dtype=torch.float64)
            coords.append(coord)

        # Create meshgrid of positions
        position_grids = torch.meshgrid(*coords, indexing="ij")

        # For each frequency index, compute the periodogram
        for i, idx in enumerate(indices):
            ...
            # Compute k^T * x for each position x
            # position_grids is a list of tensors, each with shape grid_shape
            # We need to compute the dot product of freq_vec with the position at each point
            dot_product = torch.zeros(grid_shape, dtype=torch.float64)
            for dim in range(ndim):
                dot_product += freq_vec[dim] * position_grids[dim]

            # Compute the DFT: sum_x z(x) * exp(-i * k^T * x)
            dft = torch.sum(values * torch.exp(-1j * dot_product))

            # Periodogram is |DFT|^2 / n
            periodogram_values[i] = (torch.abs(dft) ** 2) / n_points

        return periodogram_values

    def compute_expected_periodogram_at_indices(
        self, model: CovarianceModel, indices: List[tuple[int, ...]]
    ) -> torch.Tensor:
        """
        Compute the expected periodogram at specific frequency indices without using FFT.
        ...
        This method computes this directly for the specified frequency indices,
        ...
        for i, idx in enumerate(indices):
            ...
            omega_dot_r = torch.sum(
                freq_vec.unsqueeze(1) * lags_flat, dim=0
            )  # (n_lags,)
    ...
    def __call__(
        ...
        """
        # Compute periodogram at the specified indices (without FFT)
        p = self.compute_periodogram_at_indices(sample, indices)

        # Compute expected periodogram at the specified indices (without FFT)
        ep = self.compute_expected_periodogram_at_indices(model, indices)
...
class SGDTrainer:
    ...
    >>> sample = sampler()
    >>>
    >>> # Create stochastic Whittle (note: no periodogram/expected_periodogram objects needed)
        ...
        for param_name in self.model.free_parameter_names:
            ...
            if param_value is not None:
                ...
                if isinstance(current_value, torch.Tensor):
                    self._original_params[param_name_only] = (
                        current_value.detach().item()
                    )
                else:
    ...
    def get_parameter_values(self) -> dict:
        ...
        """
        return {
            name: self.model.get_parameter(name)
            for name in self.model.free_parameter_names
        }
