...
def func(
    ...
    post = mcmc.RW_MH(
        mcmc_niter, acceptance_lag=acceptance_lag, approx_grad=approx_grad
    )[burnin:]
    ...
    if name in {"DeWhittle", "Whittle"}:
        ...
        ll.compute_C3(
            ll.res.x
        )  # TODO: change C # TODO: for C3 can get singular matrix error!!

        adj_post = mcmc.RW_MH(
            mcmc_niter, adjusted=True, acceptance_lag=acceptance_lag, C=ll.C3
        )[burnin:]
    else:
...
def main():
    ...
    likelihood_name, n1, n_datasets = (
        sys.argv[1],
        int(sys.argv[2]),
        int(sys.argv[3]),
    )  # argument for C3? # TODO: make args string?
    ...
    print(
        f"{sys.argv[0]} run with likelihood={likelihood.__name__} with {n=} and {n_datasets=}"
    )
    ...
    prior_cov = np.array(
        [[1.0, 0.0], [0.0, 0.1]]
    )  # TODO: PRIOR (VARIANCE) VERY IMPORTANT FOR COVERAGES/QUANTILES
    # prior = GaussianPrior(prior_mean, prior_cov)   # make sure sigma not negative
    ...
    with open("submit_coverages.sh", "r") as f:
        ...
        idx = text.find("ncpus=")
        nprocesses = (
            int(text[idx + 6 : idx + 8]) - 2
        )  # TODO: change when ncpus<100!! -2 to not overload error

    nprocesses = 25  # mp.cpu_count()

    with Pool(
        processes=nprocesses, initializer=init_pool_processes, maxtasksperchild=1
    ) as pool:
        for i, result in enumerate(
            pool.imap(g, range(n_datasets))
        ):  # could do imap_unordered
            params, q, q_adj, probs, probs_adj = (
                result  # TODO: save parameters as well!!!
            )
            ...
            with open(file_name, "a+") as f:
                if new_file:
                    f.write(
                        f"# prior_mean={prior_mean.tolist()}, prior_cov={prior_cov.tolist()}\n"
                    )
