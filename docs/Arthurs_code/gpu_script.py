...
for i_sample in range(n_samples):
    ...
    Jhat = debiased_whittle.jmatrix_sample(estimate, params_for_gradient, n_sims=1000, block_size=1)
    if ADJUSTED:
    ...
    h = 1 / 2 * debiased_whittle.fisher(model, Parameters((model.rho, model.sigma))) * grid_france.n_points
    if ADJUSTED:
