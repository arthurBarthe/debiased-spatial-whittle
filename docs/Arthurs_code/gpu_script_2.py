...
prior = multivariate_normal([12.0, 1.2], np.array([[1.5, 0.0], [0.0, 0.0625]]))
ADJUSTED = True
C = "C2"
...
def sim_MLEs(
    ...
    opt_options = dict(
        ftol=np.finfo(float).eps * 1e7, gtol=1e-10
    )  # moderate accuracy when simulating mles
    ...
    if ADJUSTED:
        ...
        if C == "C2":
            ...
            Jhat = debiased_whittle.jmatrix_sample(
                estimate, params_for_gradient, n_sims=n_sims, block_size=1
            )
