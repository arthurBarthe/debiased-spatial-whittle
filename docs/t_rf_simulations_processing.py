...
index = pd.MultiIndex.from_product(
    [param_list, grids, nus], names=["parameters", "grids", "nu"]
)
...
fig.suptitle(
    f"deWhittle MLEs $t$-random field, Exp. kernel, {axis_labels[0]}={round(params[0],2)}, "
    f"{axis_labels[1]}={round(params[1],2)}, "
    r"$\sigma^2_{\epsilon}$=0.1",
    fontsize=26,
    color="gray",
    y=1.02,
)
