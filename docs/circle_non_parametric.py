...
s1 = 256
nn = Sequential(Linear(3, 2 * s1), ReLU(), #BatchNorm1d(2 * s1),
                Linear(2 * s1, 2 * s1), ReLU(), #BatchNorm1d(2 * s1),
                Linear(2 * s1, 2 * s1), ReLU(), #BatchNorm1d(2 * s1),
                Linear(2 * s1, s1), ReLU(), #BatchNorm1d(s1),
                Linear(s1, s1), ReLU(), #BatchNorm1d(s1),
                Linear(s1, 1))
nn.train()
...
class NNmodel(AliasedSpectralModel):
    ...
    def spectral_density(self, frequencies: np.ndarray) -> np.ndarray:
        ...
        frequencies1 = torch.cat((frequencies, rho * torch.ones((frequencies.shape[0], 1))), dim=-1)
        frequencies2 = torch.cat((-frequencies, rho * torch.ones((frequencies.shape[0], 1))), dim=-1)
...
for i in range(10000):
    ...
    for j in range(10):
        ...
        mask[torch.randint(0, z.shape[0], (1000, )), torch.randint(0, z.shape[0], (1000, ))] = 1
