...
im1 = ax[0].imshow(
    z, cmap="Spectral", origin="lower", extent=[211.125, 230, 20, 38.875]
)
fig.colorbar(im1, shrink=0.5, ax=ax[0])

ax[1].set_title("simulated", fontsize=22)
im2 = ax[1].imshow(
    sim_z, cmap="Spectral", origin="lower", extent=[211.125, 230, 20, 38.875]
)
...
freq_grid = np.meshgrid(
    *(fftshift(2 * np.pi * fftfreq(_n)) for _n in N), indexing="ij"
)  # / (delta*n1)?
...
im = plt.imshow(
    fftshift(test.residuals),
    cmap="Spectral",
    origin="lower",
    extent=[-np.pi, np.pi] * 2,
)
