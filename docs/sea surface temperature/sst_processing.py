...
data = (y - np.sum(Bhat*X,axis=1)).reshape(n1,n2)   # data are now residuals, removing second-order polynomial trend
# data = (data-data.mean())/data.std()

fig, ax = plt.subplots(1,2, figsize=(20,15))
ax[0].set_title('Sea Temperature Data', fontsize=22)
im1 = ax[0].imshow(sst, cmap='binary', origin='lower', extent = [211.125, 230, 20 ,38.875])
fig.colorbar(im1, shrink=.5, ax=ax[0])

ax[1].set_title('Residuals', fontsize=22)
im2 = ax[1].imshow(data, cmap='binary', origin='lower', extent = [211.125, 230, 20 ,38.875])
...
def MaternSpectralDens(F1,F2, params, inf_sum=False):
    ...
    if inf_sum:
        return np.sum(f, axis=(1,2))[:N//2] + nugget/((2*np.pi)**2)  # np.sum(f, axis=(1,2)).reshape(n1,n2)# .flatten()[:N//2] # #   # /(2*np.pi)**2  # + nugget/((2*np.pi)**2) 
...
I = np.prod(delta)*np.abs(np.fft.fftshift(np.fft.fft2(data)))**2/(4*N*np.pi**2)    # pgram
...
def whittle(x): return whittle_2d(x, F1,F2, I.flatten()[:N//2], inf_sum, return_sum=True)

logpost  = lambda x: whittle(x) # + logprior(x)
# print(logpost(true_params))


def obj(prm): return -logpost(prm)
...
F = F[:N//2]
f_infsum = np.array([Matern_infsum((w[0]+G1)**2 + (w[1]+G2)**2, np.exp(MAP.x)) for w in F])
