...
def gauss_posterior_test():
    ...
    gauss_post = RW_MH(
        niter, gauss.res.x, gauss, compute_hessian(gauss, gauss.res.x, inv=True)
    )
    ...
    whittle_post = RW_MH(
        niter, whittle.res.x, whittle, compute_hessian(whittle, whittle.res.x, inv=True)
    )

    title = "posterior comparisons"
    legend_labels = ["gaussian", "deWhittle", "whittle"]
    plot_marginals(
        [gauss_post, dw_post, whittle_post],
        params,
        title,
        [r"log$\rho$", r"log$\sigma$"],
        legend_labels,
