...
class TestEpFullGrid:
    ...
    base_model = ExponentialModel(rho=2, sigma=1)
    bvm = BivariateUniformCorrelation(base_model, r=0.3, f=2.5)

    def test_compare_to_average_periodiogram(self):
        sampler = MultivariateSamplerOnRectangularGrid(self.bvm, self.grid, p=2)
        ...
        assert_allclose(np.real(avg_per), np.real(ep), rtol=0.2, atol=1)
